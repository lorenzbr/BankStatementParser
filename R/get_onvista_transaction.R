...
get_onvista_transaction <- function(df.pdf.page) {
  ...
  } else if (any(grepl(dividend.identifier, df.pdf$text)) 
             && !any(grepl(storno.identifier, df.pdf$text))) {
...
get_onvista_purchase <- function(df.pdf.page) {
  ...
  isin <- stringr::str_extract(df.transaction.data$text_original[position.isin], 
                               pattern = '\\w+$')
  wkn <- NA

  investment.name <- df.transaction.data$text_original[position.isin]
  investment.name <- gsub(isin, "", investment.name)
  investment.name <- gsub("\\s+"," ", investment.name)
  investment.name <- gsub(" $","", investment.name)
  ...
  strings.with.transaction.value <- strsplit(df.transaction.data$text_original[position.transaction.value], 
                                             "\\s+\\s+")[[1]]
  if (!(any(grepl("EUR/USD", strings.with.transaction.value)))) {
    ...
    if (grepl("EUR/USD", transaction.exchangerate)) {
      ...
      transaction.exchangerate <- as.numeric(sub(",", ".", transaction.exchangerate, 
                                                 fixed = TRUE))
    }
  }
  ...
  transaction.date <- strsplit(df.transaction.data$text_original[grep("^handelstag", 
                                                                      df.transaction.data$text)], 
                               "\\s+\\s+")[[1]][2]
  transaction.time <- strsplit(df.transaction.data$text_original[grep("^handelszeit", 
                                                                      df.transaction.data$text)], 
                               "\\s+\\s+")[[1]][2]

  ## Identify transaction fee (orderprovision, handelsplatzgeb.?hr, fremdspesen, 
  ## maklercourtage, b.?rsengeb.?hr)
  fee.terms <- c("orderprovision", "handelsplatzgeb.?hr", "fremdspesen", 
                 "maklercourtage", "b.?rsengeb.?hr")
  transaction.fee <- NA
...
get_onvista_sale <- function(df.pdf.page) {
  ...
  ## Sometimes there is Betrag zu ihren gunsten twice on the same page (because
  ## of taxes/veraeusserungsverlust aktien usw. then take the last row)
  end.transaction.data <- end.transaction.data[length(end.transaction.data)]
  ...
  ## Identify transaction value (takes into account EUR and USD format of transaction document)
  position.transaction.values <- grep("betrag zu ihren gunsten", df.transaction.data$text) + 1
  transaction.value.final <- 0
  for (position.transaction.value in position.transaction.values) {
    strings.with.transaction.value <- strsplit(df.transaction.data$text_original[position.transaction.value], 
                                               "\\s+\\s+")[[1]]
    if (!any(grepl("EUR/USD", strings.with.transaction.value))) {
      transaction.value.position <- which(strings.with.transaction.value == "EUR") + 1
    } else if (any(grepl("EUR/USD", strings.with.transaction.value))) {
      transaction.value.position <- 4
      transaction.exchangerate <- strings.with.transaction.value[2]
      if (grepl("EUR/USD", transaction.exchangerate)) {
    ...
    transaction.value <- sub(".", "", transaction.value, fixed = TRUE)
    transaction.value <- as.numeric(sub(",", ".", transaction.value, fixed = TRUE))
    transaction.value.final <- transaction.value.final + transaction.value
  }
  transaction.value <- transaction.value.final
...
get_onvista_dividend <- function(df.pdf.page) {
  ...
  start.transaction.data <- grep("^dividendengutschrift|^ertragsthesaurierung|^ertr.?gnisgutschrift", 
                                 df.pdf.page$text)[2] + 1
  end.transaction.data <- grep("betrag zu ihren gunsten", df.pdf.page$text) + 1
  ...
  transaction.date <- strsplit(df.transaction.data$text_original[position.quantity.and.price], 
                               "\\s+\\s+")[[1]][3]
  transaction.time <- NA
...
get_onvista_vorabpauschale <- function(df.pdf.page) {
  ...
  ## Get position of transaction
  start.transaction.data <- grep("^steuerpflichtige vorabpauschale", 
                                 df.pdf.page$text)[1] + 2
  end.transaction.data <- grep("betrag zu ihren lasten", df.pdf.page$text) + 1
...
get_onvista_stornodividend <- function(df.pdf.page) {
  ...
  ## Get position of transaction
  start.transaction.data <- grep("^storno - ertragsthesaurierung|^storno - ertr.?gnisgutschrift", 
                                 df.pdf.page$text) + 1
  end.transaction.data <- grep("storno unserer ertr.?gnisgutschrift", 
                               df.pdf.page$text) - 1
